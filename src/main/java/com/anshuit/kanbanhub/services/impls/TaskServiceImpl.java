...
public class TaskServiceImpl {
	...
	public Task getTaskByTaskDisplayId(String taskDisplayId) {
		...
		return this.getTaskByIdOptional(taskId).orElseThrow(() -> new CustomException(HttpStatus.NOT_FOUND,
				ExceptionDetailsEnum.TASK_NOT_FOUND_WITH_TASK_DISPLAY_ID, taskDisplayId));
	}
	...
	public int extractTaskIdFromTaskDisplayId(String taskDisplayId) {
		if (!taskDisplayId.startsWith(GlobalConstants.DEFAULT_TASK_DISPLAY_ID_PREFIX)) {
			throw new CustomException(HttpStatus.BAD_REQUEST,
					ExceptionDetailsEnum.TASK_DISPLAY_ID_NOT_STARTING_WITH_PREFIX);
		}
		...
		try {
			...
			throw new CustomException(HttpStatus.BAD_REQUEST,
					ExceptionDetailsEnum.TASK_ID_FROM_TASK_DISPLAY_ID_PARSING_ERROR);
		}
	}
}
